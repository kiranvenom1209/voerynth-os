...
const ResidentItem = ({ person, idx }) => {
  ...
  return (
    <div
      className={`p-4 rounded-lg border transition-all duration-500 opacity-0 animate-[slideUpFade_0.6s_ease-out_forwards] ${
        isHome 
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
          isHome ? 'bg-emerald-500/20' : 'bg-slate-700'
        }`}>
          {renderPersonIcon(
            person.name,
            {
...
const ResidentsCard = ({ delay = 400, editMode = false, onEditClick = null, cardId = null }) => {
  ...
  const residents = [
    ...
    { name: 'Ayanthiara', entity: 'person.ayanthiara', phone: 'sensor.xiaomi_pad_5_battery' },
